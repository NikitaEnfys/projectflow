...
export function MilestoneManager({ projectId, initialMilestones, canManage }: { projectId: string; initialMilestones: Milestone[]; canManage: boolean }) {
  ...
  async function submit(e:React.FormEvent){
    ...
    try{
      setBusy(true);
      const r=await fetch(`/api/projects/${projectId}/milestones`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name,description,dueDate,status})});
  ...
  async function saveEdit() {
    ...
    try {
      const r = await fetch(`/api/projects/${projectId}/milestones/${editingId}`, {
  ...
  async function remove(id: string, title: string) {
    ...
    setBusy(true); setError("");
    try {
      const r = await fetch(`/api/projects/${projectId}/milestones/${id}`, { method: "DELETE" });
      if (!r.ok) throw new Error(await readError(r));
