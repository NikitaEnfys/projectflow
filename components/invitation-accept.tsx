...
export function InvitationAccept({ token }: { token: string }) {
  ...
  async function accept() { setBusy(true); setError(""); const r = await fetch(`/api/invitations/${token}/accept`, { method: "POST" }); if (!r.ok) { const d = await r.json().catch(() => null); setError(d?.error || "Nem sikerült elfogadni a meghívást."); setBusy(false); return; } router.replace("/dashboard"); router.refresh(); }
