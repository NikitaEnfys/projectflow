...
export function TaskKanban({ projectId, tasks, members, milestones, canManage, clientViewer, currentUserId }: Props) {
  ...
  return <section className="mb-8 rounded-xl border p-6">
    ...
    {canManage && <form onSubmit={createTask} className="mb-7 grid gap-3 rounded-lg border p-4 md:grid-cols-2 xl:grid-cols-4">
      <input className="rounded-lg border bg-transparent p-3 xl:col-span-2" placeholder="Feladat címe" value={form.title} onChange={e=>setForm({...form,title:e.target.value})}/>
      <select className="rounded-lg border bg-transparent p-3" value={form.priority} onChange={e=>setForm({...form,priority:e.target.value})}><option value="LOW">Alacsony</option><option value="MEDIUM">Közepes</option><option value="HIGH">Magas</option><option value="URGENT">Sürgős</option></select>
      <input type="date" className="rounded-lg border bg-transparent p-3" value={form.dueDate} onChange={e=>setForm({...form,dueDate:e.target.value})}/>
      <textarea className="rounded-lg border bg-transparent p-3 md:col-span-2" rows={2} placeholder="Leírás" value={form.description} onChange={e=>setForm({...form,description:e.target.value})}/>
      <select className="rounded-lg border bg-transparent p-3" value={form.assigneeId} onChange={e=>setForm({...form,assigneeId:e.target.value})}>
        <option value="">Nincs felelős</option>{members.map(m=><option key={m.id} value={m.id}>{m.name} · {ROLE_LABELS[m.role] ?? m.role}</option>)}
      </select>
      <select className="rounded-lg border bg-transparent p-3" value={form.milestoneId} onChange={e=>setForm({...form,milestoneId:e.target.value})}><option value="">Nincs mérföldkő</option>{milestones.map(m=><option key={m.id} value={m.id}>{m.name}</option>)}</select>
      ...
      <button disabled={busy || !form.title.trim()} className="rounded-lg bg-white px-4 py-2 font-medium text-black disabled:opacity-50">+ Feladat</button>
    </form>}

    <div className="grid gap-4 xl:grid-cols-5">
      {COLUMNS.map(([status,label]) => <div key={status} className="min-w-0 rounded-lg border bg-black/10 p-3" onDragOver={e=>e.preventDefault()} onDrop={e=>{const id=e.dataTransfer.getData("text/task-id"); const t=tasks.find(x=>x.id===id); if(t) moveTask(t,status);}}>
        ...
        <div className="grid gap-3">{byStatus[status]?.map(task => {
          ...
          return <article key={task.id} draggable={movable && !busy} onDragStart={e=>e.dataTransfer.setData("text/task-id",task.id)} className="rounded-lg border bg-black/20 p-3">
            ...
            {movable && <select disabled={busy} className="mt-3 w-full rounded border bg-transparent p-2 text-xs" value={task.status} onChange={e=>moveTask(task,e.target.value)}>{COLUMNS.map(([s,l])=><option key={s} value={s}>{l}</option>)}</select>}
          </article>})}</div>
      </div>)}
    </div>
    ...
    {selected && <div className="mt-6 rounded-xl border p-5">
      ...
      {canManage ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <input className="rounded-lg border bg-transparent p-3 xl:col-span-2" value={edit.title} onChange={e=>setEdit({...edit,title:e.target.value})}/>
        <select className="rounded-lg border bg-transparent p-3" value={edit.status} onChange={e=>setEdit({...edit,status:e.target.value})}>{COLUMNS.map(([s,l])=><option key={s} value={s}>{l}</option>)}</select>
        <select className="rounded-lg border bg-transparent p-3" value={edit.priority} onChange={e=>setEdit({...edit,priority:e.target.value})}><option value="LOW">Alacsony</option><option value="MEDIUM">Közepes</option><option value="HIGH">Magas</option><option value="URGENT">Sürgős</option></select>
        <textarea className="rounded-lg border bg-transparent p-3 md:col-span-2" rows={3} value={edit.description} onChange={e=>setEdit({...edit,description:e.target.value})}/>
        <input type="date" className="rounded-lg border bg-transparent p-3" value={edit.dueDate} onChange={e=>setEdit({...edit,dueDate:e.target.value})}/>
        <select className="rounded-lg border bg-transparent p-3" value={edit.assigneeId} onChange={e=>setEdit({...edit,assigneeId:e.target.value})}><option value="">Nincs felelős</option>{members.map(m=><option key={m.id} value={m.id}>{m.name} · {ROLE_LABELS[m.role] ?? m.role}</option>)}</select>
        <select className="rounded-lg border bg-transparent p-3" value={edit.milestoneId} onChange={e=>setEdit({...edit,milestoneId:e.target.value})}><option value="">Nincs mérföldkő</option>{milestones.map(m=><option key={m.id} value={m.id}>{m.name}</option>)}</select>
        ...
        <div className="flex gap-2"><button disabled={busy || !edit.title.trim()} onClick={saveEdit} className="rounded-lg bg-white px-4 py-2 text-black disabled:opacity-50">Mentés</button><button disabled={busy} onClick={()=>removeTask(selected)} className="rounded-lg border px-4 py-2">Törlés</button></div>
      ...
      <div className="mt-6 border-t pt-5">
        ...
        <div className="mt-3 grid gap-3">
          {selected.comments.length === 0 ? <p className="text-sm text-gray-500">Még nincs komment.</p> : selected.comments.map(c => <article key={c.id} className="rounded-lg border p-3">
            <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-medium">{c.author.name}</p><p className="mt-1 whitespace-pre-wrap text-sm text-gray-700 dark:text-gray-300">{c.content}</p></div>{(canManage || c.authorId === currentUserId) && <button disabled={busy} onClick={()=>deleteComment(c.id)} className="text-xs text-gray-500 hover:underline">Törlés</button>}</div>
        ...
        <form onSubmit={addComment} className="mt-4 grid gap-3 md:grid-cols-[1fr_auto_auto]">
          <textarea rows={2} maxLength={5000} className="rounded-lg border bg-transparent p-3" placeholder="Új komment…" value={comment} onChange={e=>setComment(e.target.value)}/>
          {!clientViewer && <select className="rounded-lg border bg-transparent p-3" value={commentVisibility} onChange={e=>setCommentVisibility(e.target.value)}><option value="INTERNAL">Belső</option>{selected.clientVisible && <option value="CLIENT_VISIBLE">Ügyfélnek látható</option>}</select>}
          <button disabled={busy || !comment.trim()} className="rounded-lg bg-white px-4 py-2 text-black disabled:opacity-50">Komment</button>
        </form>
      </div>
    </div>}
  </section>;
