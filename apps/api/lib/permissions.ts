...
import { clientContactRepository, clientRepository, organizationMemberRepository, projectMemberRepository, projectRepository, taskRepository } from "@/lib/repositories";
...
export async function canManageProject(userId: string, projectId: string) {
  const project = await projectRepository.findUnique({ where: { id: projectId }, select: { organizationId: true } });
  if (!project) return false;
  const projectMembership = await getProjectMembership(userId, projectId);
...
export async function isClientProjectViewer(userId: string, projectId: string) {
  ...
  const [membership, clientContact, organizationMembership] = await Promise.all([
    ...
    clientContactRepository.findFirst({ where: { userId, clientId: project.clientId }, select: { id: true } }),
