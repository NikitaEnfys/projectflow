...
export async function PATCH(
  ...
  if (
    ...
    return json(
      {
        error:
          "Nincs jogosultságod ennek a kommentnek a szerkesztéséhez.",
      },
      { status: 403 },
    );
  }
  ...
  const updated =
    await taskCommentRepository.update({
      where: {
        id: commentId,
      },
      data: {
...
export async function DELETE(
  ...
  if (
    ...
    return json(
      {
        error:
          "Nincs jogosultságod ennek a kommentnek a törléséhez.",
      },
      { status: 403 },
    );
  }
