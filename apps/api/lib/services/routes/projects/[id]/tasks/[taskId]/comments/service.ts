...
async function getVisibleTask(
  ...
  const task =
    await taskRepository.findFirst({
      where: {
        id: taskId,
        projectId,
        ...(clientViewer
...
export async function GET(
  ...
  const comments =
    await taskCommentRepository.findMany({
      where: {
        taskId,
        ...(clientViewer
          ? {
...
export async function POST(
  ...
  if (!task) {
    return json(
      {
        error:
          "A feladat nem található vagy nem látható számodra.",
      },
      { status: 404 },
    );
  }
  ...
  if (
    ...
    return json(
      {
        error:
          "A komment legfeljebb 5000 karakter lehet.",
      },
      { status: 400 },
    );
  }
  ...
  if (clientViewer) {
    ...
    if (!task.clientVisible) {
      return json(
        {
          error:
            "Csak ügyfél számára látható feladathoz írható ügyfélnek látható komment.",
        },
        { status: 400 },
      );
    }
