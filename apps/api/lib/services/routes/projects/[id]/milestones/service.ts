...
export async function GET(_req: Request, { params }: Context) {
  ...
  const milestones = await milestoneRepository.findMany({ where: { projectId: id }, orderBy: [{ dueDate: "asc" }, { createdAt: "asc" }] });
...
export async function POST(req: Request, { params }: Context) {
  try {
    ...
    return json({ error: "Nem sikerült létrehozni a mérföldkövet.", details: error instanceof Error ? error.message : "Ismeretlen hiba" }, { status: 500 });
  }
}
