...
import { organizationMemberRepository, projectMemberRepository, runInTransaction } from "@/lib/repositories";
...
async function actor(userId: string, organizationId: string) {
  return organizationMemberRepository.findUnique({ where: { organizationId_userId: { organizationId, userId } } });
}
...
export async function PATCH(request: Request, { params }: Ctx) {
  ...
  if (!a || ![OrganizationRole.OWNER, OrganizationRole.ADMIN].includes(a.role)) return json({ error: "Nincs jogosultságod." }, { status: 403 });
  ...
  const target = await organizationMemberRepository.findFirst({ where: { id: memberId, organizationId }, include: { user: true } });
  if (!target) return json({ error: "Tag nem található." }, { status: 404 });
  if (target.role === OrganizationRole.OWNER) return json({ error: "A tulajdonos szerepköre itt nem módosítható." }, { status: 400 });
  if (a.role === OrganizationRole.ADMIN && (target.role === OrganizationRole.ADMIN || role === OrganizationRole.ADMIN)) return json({ error: "Adminisztrátori szerepkört csak a tulajdonos kezelhet." }, { status: 403 });
...
export async function DELETE(request: Request, { params }: Ctx) {
  ...
  const target = await organizationMemberRepository.findFirst({ where: { id: memberId, organizationId }, include: { user: true } });
  if (!target) return json({ error: "Tag nem található." }, { status: 404 });
  if (target.role === OrganizationRole.OWNER) return json({ error: "A szervezet tulajdonosa nem távolítható el." }, { status: 400 });
  if (a.role === OrganizationRole.ADMIN && target.role === OrganizationRole.ADMIN) return json({ error: "Adminisztrátort csak a tulajdonos távolíthat el." }, { status: 403 });
  ...
  for (const managed of managedProjects) {
    ...
    if (!replacement) {
      return json({ error: "A tag nem távolítható el, mert legalább egy projekt egyetlen projektvezetője." }, { status: 400 });
    }
