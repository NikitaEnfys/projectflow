...
import { clientRepository, organizationInvitationRepository, organizationMemberRepository, userRepository } from "@/lib/repositories";
...
export async function POST(request: Request, { params }: Ctx) {
  ...
  const role = typeof body.role === "string" && INVITABLE.has(body.role as OrganizationRole)
    ...
    : null;
  if (!email || !role) return json({ error: "Érvényes e-mail és szerepkör szükséges." }, { status: 400 });
  ...
  if (role === OrganizationRole.CLIENT) {
    ...
    const client = await clientRepository.findFirst({ where: { id: clientId, organizationId }, select: { id: true } });
    if (!client) return json({ error: "A kiválasztott ügyfél nem ehhez a szervezethez tartozik." }, { status: 400 });
  }
  ...
  if (existingUser) {
    ...
    if (existingMembership) return json({ error: "Ez a felhasználó már tagja a szervezetnek." }, { status: 409 });
  }
  ...
  return json({ ...invitation, clientName: invitation.client?.name ?? null }, { status: 201 });
}
...
export async function DELETE(request: Request, { params }: Ctx) {
  ...
  const invitation = await organizationInvitationRepository.findFirst({ where: { id: invitationId, organizationId } });
