...
export async function POST(req: Request) {
  ...
  if (!isOwner && user.memberships.length > 0) return json({ error: "Nincs jogosultságod új szervezet létrehozásához." }, { status: 403 });
  const body = await req.json();
  const name = typeof body.name === "string" ? body.name.trim() : "";
  if (!name) return json({ error: "A szervezet neve kötelező." }, { status: 400 });
  const organization = await organizationRepository.create({ data: { name, members: { create: { userId: user.id, role: "OWNER" } } } });
