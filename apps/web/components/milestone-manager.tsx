...
export function MilestoneManager({ projectId, initialMilestones, canManage }: { projectId: string; initialMilestones: Milestone[]; canManage: boolean }) {
  ...
  return <section className="pf-card mb-7 p-5 sm:p-6">
    ...
    {canManage ? <form onSubmit={submit} className="mb-6 grid gap-3 rounded-xl border border-[#e8ebf1] bg-[#fbfcff] p-4 md:grid-cols-2"><input className="rounded-xl border bg-white p-3" placeholder="Mérföldkő neve" value={name} onChange={e=>setName(e.target.value)} /><input type="date" className="rounded-xl border bg-white p-3" value={dueDate} onChange={e=>setDueDate(e.target.value)} /><textarea className="rounded-xl border bg-white p-3 md:col-span-2" placeholder="Leírás" value={description} onChange={e=>setDescription(e.target.value)} rows={2}/><select className="rounded-xl border bg-white p-3" value={status} onChange={e=>setStatus(e.target.value)}><option value="PLANNED">Tervezett</option><option value="IN_PROGRESS">Folyamatban</option><option value="COMPLETED">Kész</option></select><button disabled={busy||!name.trim()} className="rounded-lg bg-[#5b67f1] px-4 py-2 font-medium text-white disabled:opacity-50">{busy?"Mentés...":"+ Mérföldkő"}</button></form> : null}
    {initialMilestones.length===0 ? <div className="rounded-xl border border-[#e8ebf1] bg-[#fbfcff] p-4 text-sm text-gray-600">Még nincs mérföldkő.</div> : <div className="grid gap-3">{initialMilestones.map(m => editingId === m.id ? <div key={m.id} className="grid gap-3 rounded-xl border border-[#e8ebf1] bg-[#fbfcff] p-4 md:grid-cols-2"><input className="rounded-xl border bg-white p-3" value={editName} onChange={e=>setEditName(e.target.value)} /><input type="date" className="rounded-xl border bg-white p-3" value={editDueDate} onChange={e=>setEditDueDate(e.target.value)} /><textarea className="rounded-xl border bg-white p-3 md:col-span-2" value={editDescription} onChange={e=>setEditDescription(e.target.value)} rows={2}/><select className="rounded-xl border bg-white p-3" value={editStatus} onChange={e=>setEditStatus(e.target.value)}><option value="PLANNED">Tervezett</option><option value="IN_PROGRESS">Folyamatban</option><option value="COMPLETED">Kész</option></select><div className="flex gap-2"><button type="button" disabled={busy || !editName.trim()} onClick={saveEdit} className="rounded-lg bg-[#5b67f1] px-4 py-2 text-white disabled:opacity-50">Mentés</button><button type="button" disabled={busy} onClick={()=>setEditingId(null)} className="rounded-lg border px-4 py-2">Mégse</button></div></div> : <div key={m.id} className="rounded-xl border border-[#e8ebf1] bg-[#fbfcff] p-4"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-semibold">{m.name}</p><p className="mt-1 text-sm text-gray-600">{m.description||"Nincs leírás."}</p></div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full border px-3 py-1 text-xs">{STATUS_LABELS[m.status]||m.status}</span>{canManage && <><button type="button" onClick={()=>beginEdit(m)} className="rounded-lg border px-3 py-1 text-xs">Szerkesztés</button><button type="button" disabled={busy} onClick={()=>remove(m.id,m.name)} className="rounded-lg border px-3 py-1 text-xs disabled:opacity-50">Törlés</button></>}</div></div><p className="mt-3 text-xs text-gray-500">Határidő: {m.dueDate ? new Date(m.dueDate).toLocaleDateString("hu-HU") : "nincs megadva"}</p></div>)}</div>}
  </section>;
}
