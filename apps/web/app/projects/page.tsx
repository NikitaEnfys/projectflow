      ...
      <div className="pf-page-header">
        <div>
          ...
          <p className="pf-subtitle">
            Minden projekted, felelősöd és határidőd egy átlátható nézetben.
          </p>
        </div>
        <Link href="/projects/new" className="pf-button-primary">
          <span className="text-lg leading-none">+</span> Új projekt
        </Link>
      </div>

      {projects.length === 0 ? (
        <div className="pf-empty">
          <p className="font-semibold text-[#424b5e]">Még nincs elérhető projekt.</p>
          <p className="mt-1 text-sm">Ha van jogosultságod, hozz létre egy új projektet.</p>
        </div>
      ) : (
        <div className="grid gap-4 lg:grid-cols-2 2xl:grid-cols-3">
          {projects.map((project) => (
            <article key={project.id} className="pf-card p-5 sm:p-6">
              ...
              <div className="mt-5 grid grid-cols-2 gap-3 border-t border-[#edf0f5] pt-4 text-xs">
                ...
                <div>
                  <p className="text-[#9aa2b1]">Előrehaladás</p>
                  <p className="mt-1 font-semibold text-[#515a6d]">
                    {project.progress}%
                  </p>
                </div>
