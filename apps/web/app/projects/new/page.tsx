...
export default function NewProjectPage() {
  ...
  useEffect(() => {
    async function loadOptions() {
      try {
        ...
        const [clientsRes, usersRes] = await Promise.all([apiFetch("/api/clients"), apiFetch("/api/users")]);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await apiFetch("/api/projects", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, description, clientId, ownerId, status, priority, startDate, dueDate }) });
